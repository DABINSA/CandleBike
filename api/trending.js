...
// ── 미국(야후) ──
// count: 반환 개수(랜덤 후보 풀은 더 넓게 받기 위해 50까지).
async function usTrending(isVol, count = 8) {
  const scr = isVol ? 'most_actives' : 'day_gainers';
  const fetchN = Math.min(50, Math.max(count, 25));
  const j = await yget(`https://query1.finance.yahoo.com/v1/finance/screener/predefined/saved?count=${fetchN}&scrIds=${scr}`);
  ...
  if (isVol) {
    ...
    }).sort((a, b) => b._val - a._val).slice(0, count).map(({ _val, ...x }) => x);
  }
  return pickUsMajor(quotes).slice(0, count).map((x) => {
    const chg = x.regularMarketChangePercent != null ? +(+x.regularMarketChangePercent).toFixed(1) : null;
...
// ── 한국(네이버) ──
// count: 반환 개수(랜덤 후보 풀은 더 넓게 받기 위해 50까지).
async function krTrending(isVol, count = 12) {
  const sort = isVol ? 'quantTop' : 'up';
  const pageSize = Math.min(50, Math.max(count, 12));
  const hdr = {
    'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15',
    Referer: 'https://m.stock.naver.com/',
  };
  async function market(cat) {
    const r = await withTimeout(`https://m.stock.naver.com/api/stocks/${sort}/${cat}?page=1&pageSize=${pageSize}`, { headers: hdr }, 6000);
  ...
  if (isVol) all = all.sort((a, b) => b._v - a._v).slice(0, count);
  else all = all.filter((x) => x.change != null).sort((a, b) => b.change - a.change).slice(0, count);
...
export default async function handler(req, res) {
  ...
  // count: 랜덤 후보 풀(getTrendingPool)은 count=50 으로 넓게 요청. 기본은 기존 동작(us 8 / kr 12).
  const count = Math.min(50, Math.max(1, parseInt(q.count, 10) || (isUs ? 8 : 12)));
  try {
    const list = isUs ? await usTrending(isVol, count) : await krTrending(isVol, count);
